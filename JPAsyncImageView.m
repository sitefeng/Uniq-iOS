...
//
//  https://github.com/nicklockwood/JPAsyncImageView
//
...
static NSString *const JPAsyncImageLoadDidFinish = @"apJPAsyncImageLoadDidFinish";
static NSString *const JPAsyncImageLoadDidFail = @"apAsyncImageLoadDidFail";

static NSString *const JPAsyncImageImageKey = @"apimage";
static NSString *const JPAsyncImageURLKey = @"apURL";
static NSString *const JPAsyncImageCacheKey = @"apcache";
static NSString *const JPAsyncImageErrorKey = @"aperror";


@interface JPAsyncImageConnection : NSObject
...
- (JPAsyncImageConnection *)initWithURL:(NSURL *)URL
                                cache:(NSCache *)cache
							   target:(id)target
							  success:(SEL)success
							  failure:(SEL)failure;
...
@implementation JPAsyncImageConnection
...
- (void)loadFailedWithError:(NSError *)error
{
	self.loading = NO;
	self.cancelled = NO;
    [[NSNotificationCenter defaultCenter] postNotificationName:JPAsyncImageLoadDidFail
                                                        object:self.target
                                                      userInfo:@{JPAsyncImageURLKey: self.URL,
                                                                JPAsyncImageErrorKey: error}];
}
...
- (void)cacheImage:(UIImage *)image
{
	if (!self.cancelled)
	{
		...
		NSMutableDictionary *userInfo = [NSMutableDictionary dictionaryWithObjectsAndKeys:
										 image, JPAsyncImageImageKey,
										 self.URL, JPAsyncImageURLKey,
										 nil];
		if (self.cache)
		{
			userInfo[JPAsyncImageCacheKey] = self.cache;
		}
		
		self.loading = NO;
		[[NSNotificationCenter defaultCenter] postNotificationName:JPAsyncImageLoadDidFinish
															object:self.target
	...
	{	
		if (!self.cancelled)
		{
			...
			else
			{
                @autoreleasepool
                {
                    NSError *error = [NSError errorWithDomain:@"JPAsyncImageLoader" code:0 userInfo:@{NSLocalizedDescriptionKey: @"Invalid image data"}];
...
- (void)start
{
    ...
    NSURLRequest *request = [NSURLRequest requestWithURL:self.URL
                                         ...
                                         timeoutInterval:[JPAsyncImageLoader sharedLoader].loadingTimeout];
...
@implementation JPAsyncImageLoader

+ (JPAsyncImageLoader *)sharedLoader
{
	static JPAsyncImageLoader *sharedInstance = nil;
	if (sharedInstance == nil)
	{
		sharedInstance = [(JPAsyncImageLoader *)[self alloc] init];
	}
	...
	if ((self = [super init]))
	{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
												 selector:@selector(imageLoaded:)
													 name:JPAsyncImageLoadDidFinish
												   object:nil];
		[[NSNotificationCenter defaultCenter] addObserver:self
												 selector:@selector(imageFailed:)
													 name:JPAsyncImageLoadDidFail
												   object:nil];
	}
	return self;
}
...
- (void)updateQueue
{
    //start connections
    NSUInteger count = 0;
    for (JPAsyncImageConnection *connection in self.connections)
    {
...
{  
    ...
    NSURL *URL = (notification.userInfo)[JPAsyncImageURLKey];
    for (NSInteger i = (NSInteger)[self.connections count] - 1; i >= 0; i--)
    {
        JPAsyncImageConnection *connection = self.connections[(NSUInteger)i];
        if (connection.URL == URL || [connection.URL isEqual:URL])
        {
            //cancel earlier connections for same target/action
            for (NSInteger j = i - 1; j >= 0; j--)
            {
                JPAsyncImageConnection *earlier = self.connections[(NSUInteger)j];
            ...
            //perform action
			UIImage *image = (notification.userInfo)[JPAsyncImageImageKey];
...
- (void)imageFailed:(NSNotification *)notification
{
    ...
    for (NSInteger i = (NSInteger)[self.connections count] - 1; i >= 0; i--)
    {
        ...
        if ([connection.URL isEqual:URL])
        {
            ...
            {
                NSError *error = (notification.userInfo)[JPAsyncImageErrorKey];
...
- (void)loadImageWithURL:(NSURL *)URL target:(id)target success:(SEL)success failure:(SEL)failure
{
    ...
    JPAsyncImageConnection *connection = [[JPAsyncImageConnection alloc] initWithURL:URL
                                                                           cache:self.cache
                                                                          target:target
                                                                         success:success
                                                                         failure:failure];
    ...
    {
        JPAsyncImageConnection *existingConnection = self.connections[i];
...
@implementation UIImageView(JPAsyncImageView)

- (void)setImageURL:(NSURL *)imageURL
{
	[[JPAsyncImageLoader sharedLoader] loadImageWithURL:imageURL target:self action:@selector(setImage:)];
}

- (NSURL *)imageURL
{
	return [[JPAsyncImageLoader sharedLoader] URLForTarget:self action:@selector(setImage:)];
}

@end


@interface JPAsyncImageView ()
...
@implementation JPAsyncImageView
...
- (void)setImageURL:(NSURL *)imageURL
{
    UIImage *image = [[JPAsyncImageLoader sharedLoader].cache objectForKey:imageURL];
    if (image)
    {
        self.image = image;
        return;
...
- (void)dealloc
{
    [[JPAsyncImageLoader sharedLoader] cancelLoadingURL:self.imageURL target:self];
}
